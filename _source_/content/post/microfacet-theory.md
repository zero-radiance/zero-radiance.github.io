...
The microfacet theory is a fundamental component of physically based rendering. Unfortunately, it is often perceived as being rather complicated. There are several reasons for this. The first one is that most papers only focus on a single component (such as the distribution of normals or the masking function), while glossing over the rest. The second problem is the lack of geometrical interpretations of mathematical expressions, which inhibits understanding at the intuitive level. Without a solid grasp of the fundamentals, engaging into any serious work on the extensions or applications (such as Smith's approximation, multiple scattering, bump roughness, and microsurface transformations) will reveal that you are inefficient (or, worse, incompetent and ineffective). The goal of this article is to collect the fragments from the literature and explain how they fit together.
