...
For a continuous surface, *the visible projected area is greater or equal to the signed projected area*. This inequality stems from self-occlusion, which eliminates the (formerly negative) contribution of back-facing microfacets. The two types of projected areas coincide if and only if the view angle is sufficiently steep, or the microsurface -- sufficiently thin, so that the latter does not extend outside the volume swept by the macrosuface translated along the view vector (see Fig. 5 above). The error can be reduced by flattening or tiling the microsurface (see Fig. 6 below); both methods effectively reduce its height relative to the dimensions of the macrosurface.
...
Furthermore, if we want to posit that Eqn. 1a and 2a are equivalent, we must prevent the  signed projected area from turning negative by restricting the view angles to those that satisfy $\small (\bm{v} \cdot \bm{n}) > 0$. In other words, *a microsurface must be facing the observer*. If that is not the case, we have a couple of options. The first one is to mirror reflect $\small \bm{v}$ across the macrosurface, which is equivalent to reversing the signs of both $\small \bm{n}$ and $\small \bm{m}$. The second option is to reverse the sign of $\small \bm{v}$. The first method can produce wildly inaccurate visibility values (which is evident if you flip a bump upside down, turning it into a cavity), and should be avoided. The alternative is usually well-behaved (especially for tiling surfaces), since it guarantees that self-occlusion is properly accounted for.
