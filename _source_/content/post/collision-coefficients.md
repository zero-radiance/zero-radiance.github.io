...
If we define the [relative permittivity](https://en.wikipedia.org/wiki/Relative_permittivity)[^9] \\(\varepsilon\_r\\) and the [relative permeability](https://en.wikipedia.org/wiki/Permeability_(electromagnetism)#Relative_permeability_and_magnetic_susceptibility) \\(\mu\_r\\) using the vacuum as reference,
...
$$ \tag{9.4}
	\big( \nabla^2 + \big( \omega / c \big)^2 \big) \psi(\bm{r}, \omega) e^{i \omega t} = \xi(\bm{r}, \omega) e^{i \omega t}.
$$
...
This implies that we only need to find the expression of \\(\bm{E}\\), and the magnetic field is then

$$ \tag{11.2}
	\bm{B}(\bm{r}, \omega) = \frac{i}{\omega} \nabla \times \bm{E}(\bm{r}, \omega).
$$

On the other hand, the second Maxwell equation depends the region under consideration:
...
$$ \tag{11.4}
\begin{aligned}
	&\nabla \times \big( \mu_1^{-1}(\omega) \bm{B}(\bm{r}, \omega) \big)
	= i \omega \varepsilon_1(\omega)  \bm{E}(\bm{r}, \omega), \cr
	&\nabla \times \big( \mu_2^{-1}(\bm{r}, \omega) \bm{B}(\bm{r}, \omega) \big)
	= i \omega \varepsilon_2(\bm{r}, \omega) \bm{E}(\bm{r}, \omega),
\end{aligned}
$$

where the first equation is only valid in the region 1, and the second equation - in the region 2.

Further, assume that the media are non-magnetic:
